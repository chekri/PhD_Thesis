function [Output]=Optimize_Deviation_from_FTL(y_init)
%Run Optimize_path_alongpath([]) in command window to start the
%optimization and it utilizes Deviation from FTL as a measure of Covered
%Volume (Chapter 9 of the thesis)
...
pp=1.0;%Multiplication factor
p1=5*pp; % 0.5 is good
p2=500.0*pp;%0,50,100, 200.0,500.0;
p3=0.0;
p4=800*pp;
...
%specified in Trajectory.m file, CTCR.m file, Trajectory.m and
%IVP_trajectory.m file (Make sure to edit them all if any changes are made)
...
%Initialization of Linear Constraints
Aeq=zeros(12*(T_steps));
%Intitialization of Bounds
...
options  = optimoptions('fmincon','Display','iter','Algorithm','interior-point','SpecifyObjectiveGradient',true,'MaxFunctionEvaluations',5000,'SpecifyConstraintGradient',true,'StepTolerance',1e-06,'OptimalityTolerance',1e-06,'ConstraintTolerance',1e-06);
nonlcon=[];
...
h=1/N;
rp1=1;
%Previous solution provided as an initial guess to the bvp to achieve quicker convergence.
Tr_prev=TRF;
U_prev=USt;
T=1;
...
for T=(1:N-1)
    ...
    Tr_current=Trajectory(U_current,F,prevsol); 
    [reach,jac_reach]=Reach_target(Rpath,U_current,Tr_current);
    [sweep,jac_sweep]=Deviation_from_FTL(U_current,Tr_current,UFL,TRF2);
...
%[reach,jac_reach]=Reach_target2(Rtar,[y(12*(N-1)+13),y(12*(N-1)+14),y(12*(N-1)+15),y(12*(N-1)+16),y(12*(N-1)+17),y(12*(N-1)+18)],Tr_current,F);%Gradient evaluation by BVP differences
[reach,jac_reach]=Reach_target(Rtar,[y(12*(N-1)+13),y(12*(N-1)+14),y(12*(N-1)+15),y(12*(N-1)+16),y(12*(N-1)+17),y(12*(N-1)+18)],Tr_current);    %Gradient evaluation by IVP differences

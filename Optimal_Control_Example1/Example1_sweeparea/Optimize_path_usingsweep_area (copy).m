function [a]=Optimize_path_usingsweep_area(y_init)
initialize() 
...
beq=zeros(12*(T_steps)+6,1);initialize() 
%R_init=[1.436,0.947,-1.263];
Rtar=[-0.174953,0.32049,1.44671]
Rtar=[-0.2,0.44,1.1]
Rtar=[0.4,-0.0,1.0]
...
[sweep,jac_sweep1,jac_sweep2]=Sweep_area(Tr_current,Tr_prev,U_current,U_prev);
sweep_area= sweep; 
reach_along=  reach*h; 
...
for T=(1:N-1)
    ...
    jac(12*T+7:12*T+12,1) = p2*jac_sweep1'+ p3*h*jac_reach';
    jac(12*(T-1)+7:12*(T-1)+12,1) =  jac(12*(T-1)+7:12*(T-1)+12,1) + p2*jac_sweep2';
    Tr_prev=Tr_current;
    U_prev=U_current;
    prevsol=Tr_current;
end

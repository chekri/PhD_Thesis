...
% This function generates the states as a solution of initial vlaue problem
% with the states at s=0 provided after solving the bvp problem. It useful
% for quicker compuation of gradients.
...
%IVP Solutions along three-tube section with initial soultions provided
[t3,p3]=ode45(@(x,y) three_tubesection(x,y,L1,L2,L3),[0 1],IC3);
sol_0=p3(end,:);

%IVP Solutions along two-tube section with the soultions of three-tube
%section at the final boundary proivided as initial state
IC2=sol_0(1:16);
[t2,p2]=ode45(@(x,y) two_tubesection(x,y,L1,L2,L3),[0 1],IC2);
sol_0=p2(end,:);


%IVP Solutions along one-tube section with the soultions of two-tube
%section at the final boundary proivided as initial state
IC1=sol_0(1:14);
...
%%         Here are those entries
      ...
      entry34 =  2.0*(u(7)*u(12) + u(6)*u(13) - u(5)*u(14));
...
%%       Components of d3
	...
	F(8) = L3*( strain3(1)*u(11)/2.0 - strain3(2)*u(10)/2.0 + strain3(3)*u(9)/2.0 -d33qn(1));
	F(9) = L3*( strain3(1)*u(10)/2.0 + strain3(2)*u(11)/2.0 - strain3(3)*u(8)/2.0 -d33qn(2));
	F(10) = L3*(-strain3(1)*u(9)/2.0 + strain3(2)*u(8)/2.0 + strain3(3)*u(11)/2.0 -d33qn(3));
	F(11) = L3*(-strain3(1)*u(8)/2.0 - strain3(2)*u(9)/2.0 - strain3(3)*u(10)/2.0 -d33qn(4));

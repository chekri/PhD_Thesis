...
!%         the fact that the unstressed state is assumed to have v=[0 0 1].
 ...
 !!Needs corrections     
   ...
   !uhat21 not included
	...
	!bt231= -U(36)*(-C2*(1/C1 + 1/C2) -C3*(1/C1))/(2.0d0*(C1+C2+C3)) 
	!bt232= -U(36)*(-C2*(1/C1) - C3*(1/C1 + 1/C3))/(2.0d0*(C1+C2+C3)) 
	bt231= U(36)/(2.0d0*C1)
	bt232= U(36)/(2.0d0*C1)
   ...
   !d mu4
	...
	b2a2=A2*uhat21*(A1*uhat11*COS(U(45)) + A3*uhat31*COS(U(45) - U(46)))/(A1+A2+A3)  &
	     + A2*uhat21*(m3(1)*uhat21*COS(U(45)) + m3(2)*uhat21*SIN(U(45)))/(A1+A2+A3)
    ...
    b3a3=A3*uhat31*(A1*uhat11*COS(U(46)) + A2*uhat21*COS(U(46) - U(45)))/(A1+A2+A3) &
    	 + A3*uhat31*(m3(1)*uhat31*COS(U(46)) + m3(2)*uhat31*SIN(U(46)))/(A1+A2+A3)
	

	DO I=1,5
  	I1=48 + 30*(I-1)
    !I1=16*I
   ! F(I1+1)=L2*(  2.0d0*u(6)*u(I1+4) + 2.0d0*u(7)*u(I1+5)  + 2.0d0*u(4)*u(I1+3) + 2.0d0*u(5)*u(I1+7))
   ! F(I1+2)=L2*( -2.0d0*u(7)*u(I1+4) + 2.0d0*u(6)*u(I1+5)  + 2.0d0*u(5)*u(I1+3) - 2.0d0*u(4)*u(I1+7))
   ! F(I1+3)=L2*( -2.0d0*u(4)*u(I1+4) - 2.0d0*u(5)*u(I1+5)  + 2.0d0*u(6)*u(I1+3) + 2.0d0*u(7)*u(I1+7))
 	!Perfect
...
!	F(I1+17)=L1*(  2.0d0*U(16+6)*U(I1+20) + 2.0d0*u(16+7)*U(I1+21)  + 2.0d0*U(16+4)*U(I1+22) + 2.0d0*U(16+5)*U(I1+23))
!    F(I1+18)=L1*( -2.0d0*u(16+7)*U(I1+20) + 2.0d0*U(16+6)*U(I1+21)  + 2.0d0*U(16+5)*U(I1+22) - 2.0d0*U(16+4)*U(I1+23))
!    F(I1+19)=L1*( -2.0d0*U(16+4)*U(I1+20) - 2.0d0*U(16+5)*U(I1+21)  + 2.0d0*U(16+6)*U(I1+22) + 2.0d0*u(16+7)*U(I1+23))
 	!Perfect
 ...
 !   F(I1+31)=L3*(  2.0d0*U(30+6)*U(I1+34) + 2.0d0*u(30+7)*U(I1+35)  + 2.0d0*U(30+4)*U(I1+21) + 2.0d0*U(30+5)*U(I1+22)) ! d d3/d q
 !   F(I1+32)=L3*( -2.0d0*u(30+7)*U(I1+34) + 2.0d0*U(30+6)*U(I1+20)  + 2.0d0*U(30+5)*U(I1+21) - 2.0d0*U(30+4)*U(I1+22)) 
 !   F(I1+33)=L3*( -2.0d0*U(30+4)*U(I1+34) - 2.0d0*U(30+5)*U(I1+20)  + 2.0d0*U(30+6)*U(I1+21) + 2.0d0*u(30+7)*U(I1+22))
...
!intb3= u1(30+8)*u1(30+4) + u1(30+9)*u1(30+5)+ u1(30+10)*u1(30+6)+ u1(30+11)*u1(30+7) + 2.0*(u1(30+1)*u1(30+12) + u1(30+2)*u1(30+13) + u1(30+3)*u1(30+14))
...
intb2= U0(8)*U0(4) + U0(9)*U0(5)+ U0(10)*U0(6)+ U0(11)*U0(7) + 2.0*(U0(1)*U0(12) + U0(2)*U0(13) + U0(3)*U0(14))
      ...
      DO I=1,5
        I1=48 + 30*(I-1)
      	...
      	!FB(I1+1)=U1(I1+1) - U0(I1+17)
      	!FB(I1+2)=U1(I1+2) - U0(I1+18)
      	!FB(I1+3)=U1(I1+3) - U0(I1+19)

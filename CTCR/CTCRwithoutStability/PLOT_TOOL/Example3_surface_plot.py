...
yy, zz = np.meshgrid(np.linspace(0,6.282,10),np.linspace(-0.2,0.2,40))
xx = zz*0 + 0.02


fig = plt.figure()
ax=plt.axes(projection='3d')
ax.plot_surface(XX, THTPLT, BIFPLT,color='m', linewidth=0, antialiased=False,alpha=0.5)
ax.set_xlabel('$F_{2}$')
...
ax.contour3D(XX, THTPLT, BIFPLT , zdir='x', levels=[0.01],offset=0.01, colors=['black'],linewidths=[2])
ax.contour3D(XX, THTPLT, BIFPLT , zdir='x', levels=[0.08],offset=0.08, colors=['black'],linewidths=[2])
ax.contour3D(XX, THTPLT, BIFPLT , zdir='x', levels=[0.12],offset=0.12, colors=['black'],linewidths=[2])
zdirs = (None, 'x', 'y', 'z', (1, 1, 0), (1, 1, 1))
ax.text(0.01, 4.7,0.15, "$F_{2}$=0.01", 'y')
...
plt.plot(THT[1],BIF[1],label='$F_{2}=0.01$')
plt.plot(THT[11],BIF[11],label='$F_{2}=0.08$')
plt.plot(THT[26],BIF[26],label='$F_{2}=0.12$')
plt.ylabel(' $m_{3}(0)$ ')
plt.xlabel('$\Theta$')
plt.title('Bifurcation diagrams for different values of $F_{2}$')
plt.legend()
plt.grid()
plt.show()
##########################################################################

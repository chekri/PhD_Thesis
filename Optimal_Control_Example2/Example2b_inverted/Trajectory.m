...
L2= u(5);
L3= u(6);
%If inital guess is not gives, start from a stright solution
if size(prevsol,1)==0
    prevsol = bvpinit(linspace(0,1,10),@mat4init);
    PrvSol = bvp4c(@(x,y)robot2rods(x,y,0,3.0,2.0,0.1,0.1,0.1,[0,0,0]), @(x,y)bcs(x,y,0,3.0,2.0,0.1,0.1,0.1,Load), prevsol);
    prevsol=PrvSol;
else 
    prevsol=bvpinit(prevsol,[0 1]);
end
%opts = bvpset('RelTol',5e-04,'AbsTol',5e-04,'Stats','on');
opts = bvpset('RelTol',1e-07,'AbsTol',1e-07);

%opts = bvpset('Stats','on');

%XYZ=bvp4c(@(x,y)robot2rods(x,y,Th1,Th2,Th3,L1,L2,L3,Load), @(x,y)bcs(x,y,Th1,Th2,Th3,L1,L2,L3,Load), prevsol);

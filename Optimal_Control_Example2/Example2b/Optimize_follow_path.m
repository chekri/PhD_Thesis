...
%specified in Trajectory.m file and in IVP_trajectory.m
%Penalizing Weights of the objective functions
p1=5.0; %              % Penalizing weight of Regularization 
p2=0.0; %40;%40;%10.0; % Penalizing weight for Covered Volume 
p3=80.0;               % Penalizing weight for deviation from prescribed path
p4=800;                % Penalizing weight for deviation from Target at final time
% Penaliing Weigths for prioritizing between Position and Orientation pursuit
mu=1.0; % 0.5 %       %Weightage for path pursuit (position) along the path
nu=0.0;%-2.5 %        %Weightage for path pursuit (orientation)along the path dot product 
                      % and that's why it is negative 
...
for T=[1:T_steps]
 ...
 %Bounds on the length parameters
 lb(12*(T-1)+10)=0;
 lb(12*(T-1)+11)=0;
 lb(12*(T-1)+12)=0;
...
nonlcon=@(U)Orientation_Constraint(U,Ori,P,d_tol_angle);
Objective(Yo,Rtar,R_init,Ori,UFL,USt,P,Tr_current,TRF2,p1,p2,p3,p4,mu,nu);
...
function [gneq,geq,gradneq,gradceq]=Orientation_Constraint(y,Ori_tar,P,d_tol_angle);

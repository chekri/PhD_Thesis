...
Rtar=[-0.2,0.44,1.1]
%Rtar=[-0.053,0.175,0.933]

%Rtar=[0.16,0.167,0.767]
Rtar=[-0.0135,0.123,0.786] %Used for example
...
USt=UFL + [0,0.1,0.2,0.0,0.3,0];
%USt=UFL + [0,0.5,1.0,0.11,0.1,-0.10];
%USt=[0.5,2.0,3.4,0.5,0.6,0.5]
%USt=[0.0,0.2,1.0,0.75,0.6,0.5];
%USt=[1.0,1.0+3.1,0.76,0.5,0.4,0.5];
%USt=UFL
P=[0,0,0];
...
if size(y_init,2)==0
    ...
    for T=(1:T_steps)
        ...
        %Yo=[Yo,[UFL(1)+0.0*rand(1)*pi,UFL(2)+0.0*(rand(1))*pi,UFL(3)+0.00*pi*rand(1),UFL(4)+0.00*(rand(1)-1)*0.3,UFL(5)+0.00*(rand(1)-1)*0.3,UFL(6)+0.00*(rand(1)-1)*0.3,0,0,0,0,0,0]];
    end
else 
    Yo=y_init;
    Yo(1:6)=[];
...
[reach,jac_reach]=Reach_targetandorientation(Rpath,Ori,[y(12*T+1),y(12*T+2),y(12*T+3),y(12*T+4),y(12*T+5),y(12*T+6)],Tr_current);
[sweep,jac_sweep]=Deviation_from_FTL(U_current,Tr_current,UFL,TRF2);

sweep_area= sweep*h; 
reach_along=  reach*h; 

jac(12+1:12+6,1) = p1*[y(12 + 7)*h;y(12 + 8)*h;y(12 + 9)*h;y(12 + 10)*h;y(12 + 11)*h;y(12 + 12)*h];
...
for T=(2:N)
    ...
    jac(12*T+1:12*T+6,1) = p1*[y(12*T + 7)*h;y(12*T + 8)*h;y(12*T + 9)*h;y(12*T + 10)*h;y(12*T + 11)*h;y(12*T + 12)*h];
    jac(12*(T-1)+7:12*(T-1)+12,1) = p2*h*jac_sweep'+ p3*h*jac_reach';
    %jac(12*(T-1)+7:12*(T-1)+12,1) =  p3*h*jac_reach';
    Tr_prev=Tr_current;
    U_prev=U_current;
...
[reach,jac_reach]=Reach_targetandorientation(Rtar,Ori,[y(12*N+1),y(12*N+2),y(12*N+3),y(12*N+4),y(12*N+5),y(12*N+6)],Tr_current);
...
jac(1:6,1) = p1*[y(7)*h/2;y(8)*h/2;y(9)*h/2;y(10)*h/2;y(11)*h/2;y(12)*h/2];
